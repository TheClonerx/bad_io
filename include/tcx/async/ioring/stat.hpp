...
namespace impl {
    ...
    struct ioring_statat_operation {
        ...
        static auto call(E &executor, tcx::uring_context auto &service, int dir_fd, char const *pathname, struct ::stat *statbuf, int flags, F &&f)
        {
            ...
            return service.async_statx(dir_fd, pathname, flags, STATX_BASIC_STATS, p, [statxbuf = std::move(statxbuf), statbuf, &executor, f = std::forward<F>(f)](tcx::uring_context auto &, io_uring_cqe const *result) mutable {
                ...
                return executor.post([f = std::move(f), result = result->res]() mutable {
                    if (result < 0)
                        return std::invoke(f, variant_type(std::in_place_index<0>, result, std::system_category()));
                    else
                        return std::invoke(f, variant_type(std::in_place_index<1>));
                });
            });
        }
    };
...
auto async_stat(E &executor, tcx::uring_context auto &service, tcx::native::c_string path, struct ::stat *statbuf, F &&f)
{
...
template <typename E, typename F>
requires tcx::completion_handler<F, tcx::impl::ioring_statat_operation::result_type>
auto async_statat(E &executor, tcx::uring_context auto &service, tcx::native::handle_type dir_fd, tcx::native::c_string path, struct ::stat *statbuf, int flags, F &&f)
{
